...
import mimetypes # For handling file types and extensions
...
# Create a separate directory for images for better organization
...
def extract_html_and_css_and_images(driver, url):
    """
    Navigates to the URL, waits for dynamic content, extracts HTML,
    downloads linked CSS and images, and extracts computed styles.
    """
    print(f"\n--- Processing: {url} ---")
    try:
        driver.get(url)
        ...
        for img_tag in image_tags:
            # Common attributes for image URLs
            img_src = img_tag.get('src')
            # Check for lazy-loading attributes
            if not img_src:
                img_src = img_tag.get('data-src') # Common for lazy loading
            if not img_src:
                img_src = img_tag.get('data-original') # Another common one
            ...
            if img_src:
                ...
                try:
                    # Use stream=True to handle larger images efficiently
                    ...
                    if content_type:
                        # Infer extension from Content-Type, e.g., 'image/jpeg' -> '.jpeg'
                        extension = mimetypes.guess_extension(content_type)
                        if not extension and 'image' in content_type: # Fallback for less common types
                            extension = '.' + content_type.split('/')[-1]
                    else:
                        extension = os.path.splitext(os.path.basename(img_url.split('?')[0]))[1]
                        if not extension:
                            extension = '.png' # Default if no extension found

                    img_filename = os.path.basename(img_url.split('?')[0])
                    # Clean filename (remove problematic chars if any) and ensure extension
                    img_filename = "".join(c for c in img_filename if c.isalnum() or c in ('.', '_', '-'))
                    if not img_filename or not img_filename.endswith(extension):
                        img_filename = f"image_{images_downloaded_count}{extension}"

                    img_file_path = os.path.join(IMAGES_DIR, img_filename)

                    with open(img_file_path, "wb") as f:
                        for chunk in img_response.iter_content(chunk_size=8192): # Stream download
                            f.write(chunk)
                    print(f"Downloaded Image: {img_filename}")
                    images_downloaded_count += 1

                    # --- Adjust the 'src' attribute in the BeautifulSoup object ---
                    # Update the img_tag's 'src' to point to the local file
                    img_tag['src'] = os.path.join("images", img_filename) # Relative path to images dir
                    # Also remove data-src or other lazy-load attributes if they were used
                    ...
                    print(f"Failed to download image from {img_url}: {req_err}")
                except Exception as e:
                    print(f"An unexpected error occurred while downloading image from {img_url}: {e}")
            else:
                # print(f"Skipping <img> tag with no 'src' or 'data-src': {img_tag}") # Uncomment for debugging
                pass # No src attribute found

        print(f"Finished downloading {images_downloaded_count} images.")
        ...
        # --- Apply Prettify for proper indentation ---
        print("Applying HTML indentation...")
        ...
        html_file_path = os.path.join(OUTPUT_DIR, "index.html")
        with open(html_file_path, "w", encoding="utf-8") as f:
            f.write(pretty_html) # Save the modified soup object back to string
...
if __name__ == "__main__":
    ...
    try:
        driver = setup_driver()
        if driver:
            extract_html_and_css_and_images(driver, TARGET_URL) 
    finally:
        if driver:
            driver.quit()
